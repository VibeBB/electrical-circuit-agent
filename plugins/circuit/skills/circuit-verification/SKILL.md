...
provides visual review evidence and `circuit_diff` provides change evidence; neither
can replace deterministic gates. Supported exports are `gerbers`, `drill`, `pos`,
`bom`, `netlist`, `pdf-sch`, `step`, `sch_pdf`, `sch_svg`, `pcb_pdf`, `pcb_svg`,
`dxf`, `ipc2581`, `odb`, `gencad`, `vrml`, and `glb`.
